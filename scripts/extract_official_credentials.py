...
Discovery is conservative: names are emitted only when observed on issuer HTML/JSON-LD.
Records remain credential-candidates until evidence promotion.
"""
...
OUT=ROOT/'credentials/extracted-candidates.jsonl'; EVID=ROOT/'evidence/extraction-evidence.jsonl'
MAX_TOTAL=12000; MAX_PAGES_PROVIDER=35; TIMEOUT=18
UA='OpenCertAtlas/2026 (+https://github.com/pavlivdevelop/OpenCertAtlas)'
MARKERS=re.compile(r'\b(certificate|certification|credential|badge|microcredential|professional certificate|skill badge|digital credential|learning path|course|training|academy|education|certif|exam|certified)\b',re.I)
    ...
    for path in PROVIDER_FILES:
        ...
        with path.open(encoding='utf-8-sig',newline='') as f:
            for r in csv.DictReader(f):
                org=(r.get('Organization') or r.get('organization') or r.get('name') or '').strip()
                url=(r.get('Official URL') or r.get('official_url') or '').strip()
                if not org or not url: continue
                k=(org.lower(),url)
                if k not in seen: seen.add(k); out.append((org,r.get('Country') or r.get('country') or 'International',url))
    return out
...
def fetch(url):
    try: r=S.get(url,timeout=TIMEOUT,allow_redirects=True)
    except requests.RequestException: return None,None
    if r.ok and 'text/html' in r.headers.get('content-type','') and len(r.text)<5_000_000: return r.url,r.text
    return None,None
    ...
    for sc in soup.select('script[type="application/ld+json"]'):
        ...
        for x in stack:
            ...
            if re.search(r'Course|Credential|Certification|LearningResource|EducationalOccupationalCredential',typ,re.I):
                ...
                if t: found.append((t,urljoin(url,x.get('url') or url),'jsonld',url,typ))
    for tag in soup.select('h1,h2,h3,h4,h5,h6'):
        t=clean(tag.get_text(' ',strip=True))
        if t and MARKERS.search(t): found.append((t,url,'heading',url,''))
    host=urlparse(url).netloc
    for a in soup.select('a[href]'):
        t=clean(a.get_text(' ',strip=True)); href=urljoin(url,a.get('href',''))
        if t and urlparse(href).netloc==host and MARKERS.search(t+' '+href): found.append((t,href,'anchor',url,''))
    out=[]; seen=set()
    for x in found:
        k=(x[0].lower(),x[1])
...
def crawl(org,country,seed):
    ...
    sm=f'{urlparse(first).scheme}://{urlparse(first).netloc}/sitemap.xml'; _,smhtml=fetch(sm)
    if smhtml:
        for u in discover(sm,smhtml,MAX_PAGES_PROVIDER*2):
            if u not in seen: seen.add(u); queue.append(u)
    results=[]
    for u in queue[:MAX_PAGES_PROVIDER]:
        final,body=(first,html) if u==first else fetch(u)
