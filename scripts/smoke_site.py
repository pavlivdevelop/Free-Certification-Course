...
FILES={name:ROOT/'site'/name for name in ('index.html','progress.html','recommend.html','pathways.html','release.html','releases.html','review.html','sources.html','promotion.html','sw.js','offline.html','robots.txt','sitemap.xml')}
def fail(message:str)->None: raise SystemExit(f"SITE SMOKE ERROR: {message}")
def main()->int:
    for name,path in FILES.items():
        if not path.exists(): fail(f"missing site/{name}")
    index=FILES['index.html'].read_text(encoding='utf-8'); progress=FILES['progress.html'].read_text(encoding='utf-8'); rec=FILES['recommend.html'].read_text(encoding='utf-8'); pathways=FILES['pathways.html'].read_text(encoding='utf-8'); release=FILES['release.html'].read_text(encoding='utf-8'); releases=FILES['releases.html'].read_text(encoding='utf-8'); review=FILES['review.html'].read_text(encoding='utf-8'); sources=FILES['sources.html'].read_text(encoding='utf-8'); promotion=FILES['promotion.html'].read_text(encoding='utf-8'); sw=FILES['sw.js'].read_text(encoding='utf-8'); robots=FILES['robots.txt'].read_text(encoding='utf-8'); sitemap=FILES['sitemap.xml'].read_text(encoding='utf-8')
    ...
    for needle,label in {'Promotion preview':'promotion review workspace','status/promotion-preview.json':'promotion payload','advisory only':'non-authoritative semantics','does not promote':'no-auto-promotion boundary','issuer page':'issuer evidence link'}.items():
        if needle.lower() not in promotion.lower(): fail(f"promotion.html missing {label}: {needle!r}")
    for needle,label in {'./pathways.html':'pathway page cache','./release.html':'release page cache','./releases.html':'release history cache','./promotion.html':'promotion page cache','./offline.html':'offline page cache','./data/pathways.json':'published pathway payload cache','./data/pathway-candidates-lite.json':'pathway candidate payload cache','./data/catalog-lite.json':'lightweight catalog cache','./status/review-queue-lite.json':'review payload cache','./status/source-health.json':'source-health payload cache','./status/release-manifest.json':'release manifest cache','./status/release-index.json':'release index cache','./status/promotion-preview.json':'promotion preview cache','./status/REVIEW-QUEUE.csv':'review CSV cache','./status/SOURCE-HEALTH.md':'source-health report cache','./status/CREDENTIAL-PATHWAY-CANDIDATES.csv':'mapping CSV cache','oca-release-indicator':'release indicator injection','manifest.json':'release metadata reference'}.items():
    ...
    for needle,label in {'<urlset':'sitemap root','https://pavlivdevelop.github.io/OpenCertAtlas/':'canonical site URL','release.html':'release URL','releases.html':'release history URL','promotion.html':'promotion URL','review.html':'review URL','sources.html':'source-health URL'}.items():
    ...
    for forbidden in ('free certification course','fcc-progress','my-certification-progress.json'):
        if forbidden in '\n'.join([index,progress,rec,pathways,release,releases,review,sources,promotion]).lower(): fail(f"legacy marker detected: {forbidden!r}")
    for name,path in FILES.items():
