...
def main()->int:
    ...
    for needle,label in {'Evidence packets':'evidence workspace heading','status/evidence-packet-index.json':'packet payload','read-only manual-review':'manual review scope','cannot change verification or price status':'read-only safety boundary','required manual checks':'review checklist','issuer page':'issuer evidence link','localStorage':'local reviewer state storage','opencertatlas:evidence-review:v2':'snapshot-bound review-state namespace','catalogSha256':'catalog snapshot binding','local review state is bound to this catalog snapshot':'snapshot safety notice','Clear local review state':'local reset control'}.items():
