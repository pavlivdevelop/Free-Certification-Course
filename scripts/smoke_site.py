...
FILES={name:ROOT/'site'/name for name in ('index.html','progress.html','recommend.html','pathways.html','sw.js')}
...
def main()->int:
    ...
    for needle,label in {'OpenCertAtlas':'canonical brand','catalog-expanded.json':'catalog data source','oca-progress':'local progress persistence','progress.html':'progress route','Learning tracks':'track navigation','recommend.html':'recommendation route','pathways.html':'pathway route'}.items():
    ...
    for needle,label in {'taxonomy/pathways.json':'pathway graph source','oca-progress':'pathway local state','prerequisite':'prerequisite semantics'}.items():
